...
KOKKOS_INLINE_FUNCTION void SumElemFaceNormal(
...
KOKKOS_INLINE_FUNCTION void
CalcElemNodeNormals(Real_t pfx[8], Real_t pfy[8], Real_t pfz[8],
...
void CalcElemVolumeDerivative(const Int_t& i,
  ...
  #pragma nounroll
  for(int j=0;j<4;j++) {
    VoluDer(x[(j+1)%4], x[(j+2)%4], x[(j+3)%4], x[(j+0)%4+4], x[(j+1)%4+4], x[(j+3)%4+4],
            y[(j+1)%4], y[(j+2)%4], y[(j+3)%4], y[(j+0)%4+4], y[(j+1)%4+4], y[(j+3)%4+4],
            z[(j+1)%4], z[(j+2)%4], z[(j+3)%4], z[(j+0)%4+4], z[(j+1)%4+4], z[(j+3)%4+4],
            dvdx(i,j), dvdy(i,j), dvdz(i,j));
    VoluDer(x[(j+3)%4+4], x[(j+2)%4+4], x[(j+1)%4+4], x[(j+0)%4], x[(j+3)%4], x[(j+1)%4],
            y[(j+3)%4+4], y[(j+2)%4+4], y[(j+1)%4+4], y[(j+0)%4], y[(j+3)%4], y[(j+1)%4],
            z[(j+3)%4+4], z[(j+2)%4+4], z[(j+1)%4+4], z[(j+0)%4], z[(j+3)%4], z[(j+1)%4],
            dvdx(i,j+4), dvdy(i,j+4), dvdz(i,j+4));
  }
}

KOKKOS_INLINE_FUNCTION
void CalcElemFBHourglassForce(const Real_t *xd,
                              const Real_t hourgam[][8], const Real_t& coefficient,
                              Real_t *hgfx) {
  Real_t hxx[4];
  ...
  for(Index_t i = 0; i < 8; i++) {
     hgfx[i] = coefficient *
               (hourgam[0][i] * hxx[0] + hourgam[1][i] * hxx[1] +
...
static inline void CalcHourglassControlForElems(Domain &domain, Real_t determ[],
  ...
  Kokkos::View<Real_t**,Kokkos::MemoryTraits<Kokkos::Unmanaged> > v_dvdx(dvdx,numElem,8);
  Kokkos::View<Real_t**,Kokkos::MemoryTraits<Kokkos::Unmanaged> > v_dvdy(dvdy,numElem,8);
  ...
  if (hgcoef > Real_t(0.)) {
    CalcFBHourglassForceForElems(domain, determ, v_x8n, v_y8n, v_z8n, v_dvdx, v_dvdy,
                                 v_dvdz, hgcoef, numElem, domain.numNode(), execSpace);
  }

  return;
}

static inline void CalcVolumeForceForElems(Domain &domain, ExecSpace execSpace) {
  Index_t numElem = domain.numElem();
  if (numElem != 0) {
...
static inline void CalcForceForNodes(Domain &domain, ExecSpace execSpace) {
  ...
  CalcVolumeForceForElems(domain, execSpace);
}
...
static inline void ApplyAccelerationBoundaryConditionsForNodes(Domain &domain, ExecSpace execSpace) {
...
KOKKOS_INLINE_FUNCTION Real_t
    CalcElemVolume(const Real_t x0, const Real_t x1, const Real_t x2,
                   const Real_t x3, const Real_t x4, const Real_t x5,
                   const Real_t x6, const Real_t x7, const Real_t y0,
                   const Real_t y1, const Real_t y2, const Real_t y3,
                   const Real_t y4, const Real_t y5, const Real_t y6,
                   const Real_t y7, const Real_t z0, const Real_t z1,
                   const Real_t z2, const Real_t z3, const Real_t z4,
                   const Real_t z5, const Real_t z6, const Real_t z7) {
...
static inline void CalcMonotonicQRegionForElems(Domain &domain, Int_t r,
                                                Real_t ptiny, ExecSpace execSpace) {
  Real_t monoq_limiter_mult = domain.monoq_limiter_mult();
  Real_t monoq_max_slope = domain.monoq_max_slope();
  Real_t qlc_monoq = domain.qlc_monoq();
  Real_t qqc_monoq = domain.qqc_monoq();

  Kokkos::parallel_for("CalcMonotonicQRegionForElems", RangePolicy(execSpace, 0, domain.regElemSize(r)),
                       KOKKOS_LAMBDA(const int i) {
...
static inline void CalcMonotonicQForElems(Domain &domain, ExecSpace execSpace){
  ...
  for(Index_t r = 0; r < domain.numReg(); ++r) {
    if (domain.regElemSize(r) > 0) {
      CalcMonotonicQRegionForElems(domain, r, ptiny, execSpace);
    }
  }
}

static void CalcQForElems(Domain& domain, ExecSpace execSpace) {
  Index_t numElem = domain.numElem();
  ...
  if (numElem != 0) {
    ...
    CalcMonotonicQGradientsForElems(domain, execSpace);
  }
}

KOKKOS_INLINE_FUNCTION
void CalcPressureForElem( Real_t& p_new_i, Real_t& bvc_i,
                          Real_t& pbvc_i, const Real_t& e_old_i,
...
CalcEnergyForElems(Real_t *p_new, Real_t *e_new, Real_t *q_new, Real_t *bvc,
  ...
  Kokkos::parallel_for ("CalcEnergyForElems", RangePolicy(execSpace, 0, length), KOKKOS_LAMBDA (const int i){
     ...
     if (delvc_i > Real_t(0.)) {
        ...
        Real_t ssc = ( pbvc_i * e_new_i
                + vnewc_e * vnewc_e * bvc_i * p_new_i ) / rho0 ;
     ...
     e_new_i = e_new_i - (  Real_t(7.0)*(p_old_i     + q_old_i)
                              - Real_t(8.0)*(pHalfStep_i + q_new_i)
                              + (p_new_i + q_tilde)) * delvc_i*sixth ;
...
static inline void CalcSoundSpeedForElems(Domain &domain, Real_t *vnewc,
  ...
  Kokkos::parallel_for("CalcSoundSpeedForElems", RangePolicy(execSpace, 0, len),
                       KOKKOS_LAMBDA(const int i) {
    Index_t ielem = domain.regElemlist(r,i);
    Real_t ssTmp =
...
static inline void EvalEOSForElems(Domain &domain, Real_t *vnewc,
  ...
  for (Int_t j = 0; j < rep; j++) {
    Kokkos::parallel_for("EvalEOSForElems A", RangePolicy(execSpace, 0, numElemReg),
      ...
      if (eosvmin != Real_t(0.)) {
        if (vnewc_ielem <= eosvmin) { /* impossible due to calling func? */
          compHalfStep[i] = compression[i];
        }
      }
      if (eosvmax != Real_t(0.)) {
        if (vnewc_ielem >= eosvmax) { /* impossible due to calling func? */
          p_old[i] = Real_t(0.);
  ...
  Kokkos::parallel_for("EvalEOSForElems F", RangePolicy(execSpace, 0, numElemReg),
                       KOKKOS_LAMBDA(const int i) {
    Index_t ielem = domain.regElemlist(r,i);
...
static inline void ApplyMaterialPropertiesForElems(Domain &domain, ExecSpace execSpace) {
  Index_t numElem = domain.numElem();
  ...
  if (numElem != 0) {
    ...
    Kokkos::parallel_for(
        "ApplyMaterialPropertiesForElems A", RangePolicy(execSpace, 0, numElem),
        KOKKOS_LAMBDA(const int i) { vnewc[i] = domain.vnew(i); });

    if (eosvmin != Real_t(0.)) {
      Kokkos::parallel_for("ApplyMaterialPropertiesForElems B", RangePolicy(execSpace, 0, numElem),
                           KOKKOS_LAMBDA(const int i) {
        if (vnewc[i] < eosvmin)
          vnewc[i] = eosvmin;
      });
    ...
    if (eosvmax != Real_t(0.)) {
      Kokkos::parallel_for("ApplyMaterialPropertiesForElems C", RangePolicy(execSpace, 0, numElem),
                           KOKKOS_LAMBDA(const int i) {
        if (vnewc[i] > eosvmax)
          vnewc[i] = eosvmax;
      });
    ...
    int error = 0;
    Kokkos::parallel_reduce("ApplyMaterialPropertiesForElems D", RangePolicy(execSpace, 0, numElem),
                         KOKKOS_LAMBDA(const int i, int& err) {
...
//      Index_t *regElemList = domain.regElemlist(r);
      ...
      EvalEOSForElems(domain, vnewc.data(), numElemReg, r, rep, execSpace);
    }
...
static inline void UpdateVolumesForElems(Domain &domain, Real_t v_cut,
  ...
  if (length != 0) {
    Kokkos::parallel_for("UpdateVolumesForElems", RangePolicy(execSpace, 0, length),
                         KOKKOS_LAMBDA(const int i) {
...
static inline void CalcCourantConstraintForElems(Domain &domain, Index_t length,
                                                 Index_t r,
                                                 Real_t qqc,
                                                 Real_t &dtcourant, ExecSpace execSpace) {
  ...
  Kokkos::parallel_reduce(
      RangePolicy(execSpace, 0, length), KOKKOS_LAMBDA(const int i, MinFinder &minf) {
...
static inline void CalcHydroConstraintForElems(Domain &domain, Index_t length,
                                               Index_t r,
                                               Real_t dvovmax,
                                               Real_t &dthydro, ExecSpace execSpace) {
  ...
  Kokkos::parallel_reduce(RangePolicy(execSpace, 0, length), KOKKOS_LAMBDA(const int i, MinFinder &minf) {
                                    Index_t indx = domain.regElemlist(r,i);

                                    if (domain.vdov(indx) != Real_t(0.)) {
                                      Real_t dtdvov =
                                          dvovmax / (FABS(domain.vdov(indx)) +
                                                     Real_t(1.e-20));
